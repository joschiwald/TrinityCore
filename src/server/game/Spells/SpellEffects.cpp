...
void Spell::EffectTriggerSpell(SpellEffIndex /*effIndex*/)
{
        ...
        && effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
    {
        ...
        switch (triggered_spell_id)
        {
            ...
            {
                // Brittle Armor
                SpellInfo const* spell = sSpellMgr->GetSpellInfo(24575, m_caster);
                if (!spell)
                    return;
            ...
            {
                // Mercurial Shield
                SpellInfo const* spell = sSpellMgr->GetSpellInfo(26464, m_caster);
                if (!spell)
                    return;
    ...
    // normal case
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(triggered_spell_id, m_caster);
    if (!spellInfo)
    {
...
void Spell::EffectForceCast(SpellEffIndex /*effIndex*/)
{
    ...
    // normal case
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(triggered_spell_id, unitTarget);
...
void Spell::EffectEnergize(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_spellInfo->Id == 45051)
    {
        ...
        {
            SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(*itr, m_caster);
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    switch (properties->Category)
    {
        ...
        case SUMMON_CATEGORY_VEHICLE:
            ...
            SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(effectInfo->CalcValue(), m_originalCaster);
...
void Spell::EffectLearnPetSpell(SpellEffIndex effIndex)
{
    ...
    SpellInfo const* learn_spellproto = sSpellMgr->GetSpellInfo(effectInfo->TriggerSpell, m_spellInfo->DifficultyId);
    if (!learn_spellproto)
        return;
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(8690, player);
    if (!spellInfo)
        return;
...
void Spell::EffectDestroyAllTotems(SpellEffIndex /*effIndex*/)
{
    ...
    for (uint8 slot = SUMMON_SLOT_TOTEM; slot < MAX_TOTEM_SLOT; ++slot)
    {
        ...
        if (totem && totem->IsTotem())
        {
            ...
            SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spell_id, m_caster);
            if (spellInfo)
            {
...
void Spell::EffectResurrectWithAura(SpellEffIndex effIndex)
{
    ...
    if (sSpellMgr->GetSpellInfo(effectInfo->TriggerSpell, m_caster))
