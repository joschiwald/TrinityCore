...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    {
        if (cInfo->spells[j] && !sSpellMgr->HasSpellInfo(cInfo->spells[j]))
        {
        ...
        for (QuestObjective const& obj : qinfo->GetObjectives())
        {
            ...
            switch (obj.Type)
            {
                ...
                case QUEST_OBJECTIVE_LEARNSPELL:
                    if (!sSpellMgr->HasSpellInfo(obj.ObjectID))
...
void ObjectMgr::LoadScripts(ScriptsType type)
{
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            {
                if (!sSpellMgr->HasSpellInfo(tmp.RemoveAura.SpellID))
                {
            ...
            {
                if (!sSpellMgr->HasSpellInfo(tmp.CastSpell.SpellID))
                {
...
void ObjectMgr::LoadEventScripts()
{
    ...
    for (uint32 i = 1; i < sSpellMgr->GetSpellInfoStoreSize(); ++i)
        if (SpellInfo const* spell = sSpellMgr->GetSpellInfo(i, DIFFICULTY_NONE))
    ...
    do
    {
        ...
        switch (creditType)
        {
            ...
            case ENCOUNTER_CREDIT_CAST_SPELL:
                if (!sSpellMgr->HasSpellInfo(creditEntry))
                {
...
{
    if (sSpellMgr->HasSpellInfo(dataN))
        return;
    ...
    if (QueryResult trainerSpellsResult = WorldDatabase.Query("SELECT TrainerId, SpellId, MoneyCost, ReqSkillLine, ReqSkillRank, ReqAbility1, ReqAbility2, ReqAbility3, ReqLevel FROM trainer_spell"))
    {
        do
        {
            ...
            for (std::size_t i = 0; i < spell.ReqAbility.size(); ++i)
            {
                ...
                if (requiredSpell && !sSpellMgr->HasSpellInfo(requiredSpell))
                {
