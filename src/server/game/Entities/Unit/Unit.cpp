...
uint32 Unit::CalcSpellResistance(Unit* victim, SpellSchoolMask schoolMask, SpellInfo const* spellInfo) const
{
    ...
    if (schoolMask & (SPELL_SCHOOL_MASK_NORMAL | SPELL_SCHOOL_MASK_HOLY))
        return 0;
    ...
    uint8 const BOSS_LEVEL = 83;
    int32 const BOSS_RESISTANCE_CONSTANT = 510;
...
int32 Unit::GetMechanicResistChance(SpellInfo const* spell) const
{
    if (!spell)
        return 0;
    int32 resist_mech = 0;
